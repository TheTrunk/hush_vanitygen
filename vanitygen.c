...
const char *version = HUSH_VANITYGEN_VERSION;
...
vg_thread_loop(void *arg)
{
	...
	while (!vcp->vc_halt) {
		...
		for (i = 0; i < nbatch; i++, vxcp->vxc_delta++) {
			...
			RIPEMD160(hash1, sizeof(hash1), &vxcp->vxc_binres[2]);  //Don't overwrite the 2nd prefix byte. 
...
{
	fprintf(stderr,
"Hush_Vanitygen %s (" OPENSSL_VERSION_TEXT ")\n"
"Usage: %s [-vqnrik1T] [-t <threads>] [-f <filename>|-] [<pattern>...]\n"
"Generates a Hush receiving transparent address matching <pattern>, and outputs the\n"
"address and associated private key.  The private key may be stored in a safe\n"
"location or imported into a Hush client to spend any balance received on\n"
"the address.\n"
...
"-T            Generate Hush testnet address\n"
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "vqnrik1eE:P:TF:t:h?f:o:s:")) != -1) { // while ((opt = getopt(argc, argv, "vqnrik1eE:P:NTXF:t:h?f:o:s:")) != -1) { //thats in z port TODO
		switch (opt) {
		case 'v':
			verbose = 2;
			break;
		...
		case '1':
			...
			break;
		case 'T':
			addrtype = 0x1d25; //0x1cb8
			privtype = 0xef; //128
			scriptaddrtype = 0x1cba; //0x1cbd
			break;
		case 'F':
